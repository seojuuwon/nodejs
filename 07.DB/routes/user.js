...
router.post('/handleJoin', (req, res)=>{
    ...
    // DB 연동 : conn.query(sql구문, (선택)sql 구문 안에 데이터, 연동 이후 콜백함수)
...
router.get('/logout', (req, res)=>{
    ...
    // Q. 왜 res.redirect('/')가 아닐까?
    // A. 메인창으로 이동할 때, 새로고침을 하고 이동, redirect는 단순한 페이지 이동, 새로고침 x
...
router.get('/select', (req, res)=>{
    // step 1. 사용자가 보내온 데이터를 확인하기 (get 방식)
    // ** post 방식으로 데이터를 받을 땐 req.body
    //    get 방식으로 데이터를 받을 땐 req.query
...
router.post('/delete', (req, res)=>{
    // step 1. 사용자가 delete.html 에서 보내온 데이터를 확인하기 (post 방식)
    //         => select router 로직 참고
